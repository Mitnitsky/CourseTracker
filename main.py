...
class mywindow(QtWidgets.QMainWindow):
    ...
    def updateGrades(self):
        ...
        if self.grades == []:
            self.errorMessage()
            return
        for i in range(0, len(self.grades[0])):
            if not self.gotGrades:
                ...
                for k in range(0, len(self.grades)):
                    self.ui.grades_table.setItem(k,i,QtWidgets.QTableWidgetItem(self.grades[k][i]))
            else:
                try:
                    self.ui.grades_table.setColumnCount(len(self.grades[0]))
                    self.ui.grades_table.setRowCount(len(self.grades))
                    newGrades = []
                    for k in range(0, len(self.grades)):
                        newGrades.append(QtWidgets.QTableWidgetItem(self.grades[k][i]))
                    if not changedInput and \
                        ((newGrades[0] and self.ui.grades_table.item(0,i).text() != newGrades[0].text()) or \
                        (newGrades[1] and self.ui.grades_table.item(1,i).text() != newGrades[1].text())):
                        notify=True
                    ...
                    for k in range(0, len(self.grades)):
                        self.ui.grades_table.setItem(k,i,newGrades[k])
                except ValueError or TypeError:
                    for k in range(0, len(self.grades)):
                        self.ui.grades_table.setItem(k,i,QtWidgets.QTableWidgetItem(newGrades[k]))
